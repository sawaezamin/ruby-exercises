...
RSpec.describe Werewolf do
  ...
  it 'when starting as a human, changing a second time makes it a werewolf' do
    werewolf = Werewolf.new('David', 'London')
  ...
  it 'consumes a victim' do
    ...
    expect(werewolf.consume(victim)).to eq("#{victim} consumed")
  end
  ...
  it 'cannot consume a victim if it is in human form' do
    ...
    expect(werewolf.consume(victim)). to eq("cannot consume")
  end
